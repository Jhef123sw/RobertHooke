...
def _generar_imagenes_reportes_por_fecha():
    cursos = [
...
def generar_todo_reporte_task():
    try:
        return _generar_todo_reporte()
    except Exception as e:
        logger.exception("Error en generar_todo_reporte_task")
        return f"Error: {str(e)}"

@shared_task
def generar_imagenes_reportes_por_fecha_task():
    try:
        return _generar_imagenes_reportes_por_fecha()
    except Exception as e:
        logger.exception("Error en generar_imagenes_reportes_por_fecha_task")
        return f"Error generando imágenes: {str(e)}"


# --- Task maestro (bind=True para usar self.update_state) ---
@shared_task(bind=True)
def generar_reportes_completos_task(self):
    try:
        # 1) inicio
        self.update_state(state='PROGRESO', meta={'step': 'Iniciando: generando reportes', 'progress': 5})

        # 2) ejecutar la generación de reportes de forma síncrona (misma ejecución)
        self.update_state(state='PROGRESO', meta={'step': 'Generando reportes', 'progress': 20})
        resultado1 = _generar_todo_reporte()  # <-- llamada síncrona al helper

        # 3) generar imágenes
        self.update_state(state='PROGRESO', meta={'step': 'Generando gráficos', 'progress': 60})
        resultado2 = _generar_imagenes_reportes_por_fecha()  # <-- síncrono

        # 4) finalizando
        self.update_state(state='PROGRESO', meta={'step': 'Finalizando', 'progress': 90})

        # opcional: delay pequeño para asegurar persistencia de estado
        time.sleep(0.5)

        return {'status': 'Completado', 'progress': 100, 'detalle': f"{resultado1} | {resultado2}"}
    except Exception as e:
        logger.exception("Error en generar_reportes_completos_task")
